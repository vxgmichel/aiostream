...
async def test_map(assert_run, event_loop):
    ...
    with event_loop.assert_cleanup():
        xs = stream.range(1, 4) | pipe.map(compat.sleep)
        expected = [None] * 3
        await assert_run(xs, expected)
    ...
    with event_loop.assert_cleanup():
        xs = stream.range(1, 4) | pipe.map(compat.sleep, task_limit=1)
        expected = [None] * 3
        await assert_run(xs, expected)
    ...
    with event_loop.assert_cleanup():
        xs = stream.range(1, 4)
        ys = xs | pipe.map(compat.sleep, xs)
    ...
    with event_loop.assert_cleanup():
        xs = stream.range(1, 4)
        ys = xs | pipe.map(compat.sleep, xs, task_limit=1)
        await assert_run(ys, [1, 2, 3])
        assert event_loop.steps == [1, 2, 3]
    ...
    with event_loop.assert_cleanup():
        ...
        ys = xs | pipe.map(compat.sleep, xs, ordered=False)
        await assert_run(ys, [1, 2, 3, 4, 5])
        assert event_loop.steps == [1, 1, 1, 1, 1]

    # Invalid argument
    with pytest.raises(ValueError):
        await (stream.range(1, 4) | pipe.map(compat.sleep, task_limit=0))

    with pytest.raises(ValueError):
        await (stream.range(1, 4)
               | pipe.map(compat.sleep, task_limit=0, ordered=False))

    # Break
    with event_loop.assert_cleanup():
        xs = stream.count(1)
        ys = xs | pipe.map(compat.sleep, xs, task_limit=10)
    ...
    with event_loop.assert_cleanup():
        xs = stream.count(1)
        ys = xs | pipe.map(compat.sleep, xs, task_limit=1) | pipe.timeout(5)
        await assert_run(ys, [1, 2, 3, 4], compat.timeout_error())

    # Force await
    with event_loop.assert_cleanup():
        xs = stream.iterate([1, 2, 3])
        ys = xs | pipe.map(async_(lambda x: compat.sleep(x, x)))
        await assert_run(ys, [1, 2, 3])
        assert event_loop.steps == [1, 1, 1]

    # Map await_
    with event_loop.assert_cleanup():
        xs = stream.iterate(map(lambda x: compat.sleep(x, x), [1, 2, 3]))
